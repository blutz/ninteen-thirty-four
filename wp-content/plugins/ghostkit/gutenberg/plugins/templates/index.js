            ...
            >
                <div className="components-modal__header">
                    <div className="components-modal__header-heading-container">
                        ...
                        <h1 id="components-modal-header-1" className="components-modal__header-heading">{ __( 'Templates', 'ghostkit' ) }</h1>
                    </div>
                    { showLoadingSpinner ? (
                            ...
                            ...( themeTemplates && themeTemplates.length ? [
                                {
                                    ...
                                    title: (
                                        <Tooltip text={ __( 'Templates from the theme.', 'ghostkit' ) }>
                                            <span>
                                                { GHOSTKIT.themeName || __( 'Theme', 'ghostkit' ) }
                                            </span>
                                        </Tooltip>
                                    ),
                                            ...
                                            <div>
                                                { 'local' === tabType ? (
                                                    <Fragment>
                                                        ...
                                                        <ExternalLink className="components-button is-button is-primary" href={ GHOSTKIT.adminTemplatesUrl }>{ __( 'Add Template', 'ghostkit' ) }</ExternalLink>
                                                    </Fragment>
                                                ) : (
                                                    __( 'No templates found.', 'ghostkit' )
                                                ) }
                                            ...
                                            <Fragment key={ `${ tabType }-${ selectedCategory }` }>
                                                <div className="ghostkit-plugin-templates-categories-row">
                                                    ...
                                                    <div className="ghostkit-plugin-templates-count">
                                                        <RawHTML>{ sprintf( __( 'Templates: %s', 'ghostkit' ), `<strong>${ currentTemplates.length }</strong>` ) }</RawHTML>
                                                    </div>
                                                </div>
                                                { this.state.error }
                                                <Masonry
...
function checkMissingBlocks( data ) {
    ...
    const missingBlocksInfo = applyFilters( 'ghostkit.templates.missingBlocksInfo', {
        'nk/awb': {
            info: __( '<strong>Advanced Backgrounds</strong> plugin is required to use background image and video blocks.', 'ghostkit' ),
            pluginUrl: 'https://wordpress.org/plugins/advanced-backgrounds/',
        },
        'nk/visual-portfolio': {
            info: __( '<strong>Visual Portfolio</strong> plugin is required to show portfolio layouts.', 'ghostkit' ),
    ...
    if ( Object.keys( missingBlocks ).length ) {
        Object.keys( missingBlocks ).forEach( ( blockName ) => {
            result.push(
                ...
                >
                    <RawHTML>{ sprintf( __( '%s block is missing.', 'ghostkit' ), `<strong>${ blockName }</strong>` ) }</RawHTML>
